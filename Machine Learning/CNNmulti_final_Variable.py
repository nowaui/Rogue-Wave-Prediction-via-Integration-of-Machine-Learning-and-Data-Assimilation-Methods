...
class CNN(nn.Module):
    def __init__(self, k, m, m_max, kernel_size=3):
        ...
        self.step_predictors = nn.ModuleList([
            nn.Linear(256, 500) for _ in range(m_max)
        ])
    ...
    def forward(self, x, m):
        ...
        outputs = []
        for i in range(m):
            output = self.step_predictors[i](x)
            outputs.append(output)
        
        return torch.stack(outputs, dim=1)               


def pre_data(csv_path, num, k, max_steps):
    ...
    for i in range(num):
        ...
        for j in range(len(u_exp) - k - max_steps + 1):
            X_sample = u_exp[j:j + k]            
            y_sample = []
            for s in range(max_steps):
                delta = u_exp[j + k + s] - u_exp[j + k + s - 1]
                y_sample.append(delta)
...
def calculate_layer_wise_cost(model, X_batch, y_batch, m, device):
    ...
    with torch.no_grad():
        ...
        x1 = F.pad(X_batch, (model.pad, model.pad), mode='circular')
        x1 = model.relu(model.conv1(x1))
        costs['conv1'] = F.mse_loss(x1, torch.zeros_like(x1)).item()
        x1p = model.pool1(x1)
        costs['pool1'] = F.mse_loss(x1p, torch.zeros_like(x1p)).item()
               
        x2 = F.pad(x1p, (model.pad, model.pad), mode='circular')
        x2 = model.relu(model.conv2(x2))
        costs['conv2'] = F.mse_loss(x2, torch.zeros_like(x2)).item()
        x2p = model.pool2(x2)
        costs['pool2'] = F.mse_loss(x2p, torch.zeros_like(x2p)).item()
               
        x3 = F.pad(x2p, (model.pad, model.pad), mode='circular')
        x3 = model.relu(model.conv3(x3))
        costs['conv3'] = F.mse_loss(x3, torch.zeros_like(x3)).item()
        x3p = model.pool3(x3)
        costs['pool3'] = F.mse_loss(x3p, torch.zeros_like(x3p)).item()
             
        x_flat = x3p.view(x3p.size(0), -1)
        x_fc1 = model.relu(model.fc1(x_flat))
        costs['fc1'] = F.mse_loss(x_fc1, torch.zeros_like(x_fc1)).item()
                                
        for i in range(m):
            step_out = model.step_predictors[i](x_fc1)
                         
            costs[f'step_{i+1}'] = F.mse_loss(step_out, y_batch[:, i, :]).item()
                        
            costs[f'fc2_step_{i+1}'] = F.mse_loss(step_out, torch.zeros_like(step_out)).item()
        return costs
...
def predict_at_time(model, u_initial, t_target, t_start, dt_frame, device, u_min, u_max, X_compare, k, m):
    ...
    while step < steps_needed:
        ...
        delta_us = model(X_tensor, m).detach().cpu().numpy().squeeze()            
        if delta_us.ndim == 1:
        ...
        for i in range(min(m, steps_needed - step)):
            ...
            if not np.any(region):
                region = None
            peak_amp_error = float(np.abs(np.max(u_pred_denorm if region is None else u_pred_denorm[region]) - np.max(u_true_denorm if region is None else u_true_denorm[region])))
            mean_abs_err_peak_region = float(np.mean(np.abs((u_pred_denorm - u_true_denorm)[region]))) if region is not None else float('nan')
...
def plot_comparison_value_at_time(t_compare, t_start, dt_frame, predictions, true_values, x, batch_size, num, idx, k, m):
    ...
    plt.plot(x, true_values[step], label=f'True u(x, t={t_compare:.2f})', linestyle='--')
    plt.xlabel('x')
    plt.ylabel('u')
    plt.title(f'Comparison at t={t_compare:.2f}, batch_size={batch_size}, m={m}, {num} Rogue Wave Experiments')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
...
def plot_comparison_recursive_steps(t_compare, t_start, dt_frame, predictions_dict, true_values_dict, x, batch_size, num, idx, k, m_values):
    step = int((t_compare - t_start) / dt_frame) - 1
    plt.figure(figsize=(10, 6))    
    plt.plot(x, true_values_dict[m_values[0]][step], label=f'True u(x, t={t_compare:.2f})', linestyle='--', color='black')
    ...
    plt.xlabel('x')
    plt.ylabel('u')
    plt.title(f'Comparison of Different m at t={t_compare:.2f}, batch_size={batch_size}, {num} Rogue Wave Experiments')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
    save_path = os.path.join(save_dir, f"wave_comparison_recursive_steps_batch_{batch_size}_exp{idx}_num{num}_t{t_compare:.2f}_k={k}.png")
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()

def plot_losses(train_losses_all, val_losses_all, num, k, m_values):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Training and Validation Loss for {num} Rogue Wave Experiments, m={m_values[-1]}')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
    save_path = os.path.join(save_dir, f"losses_num{num}_k={k}_m={m_values[-1]}.png")
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()

def plot_mse(mse_errors_all, times, num, compare_index, k, m_values):
    plt.figure()
    ...
    plt.xlabel('Time t')
    plt.ylabel('MSE')
    plt.title(f'MSE at Time, m={m_values[-1]}, {num} Rogue Wave Experiments')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
    save_path = os.path.join(save_dir, f"mse_exp{compare_index}_num{num}_k={k}_m={m_values[-1]}.png")
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()

def plot_mse_recursive_steps(mse_errors_dict, times, num, compare_index, batch_size, k, m_values):
    plt.figure(figsize=(10, 6))
    ...
    plt.xlabel('Time t')
    plt.ylabel('MSE')
    plt.title(f'MSE Comparison for Different m, batch_size={batch_size}, {num} Rogue Wave Experiments')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
    save_path = os.path.join(save_dir, f"mse_comparison_recursive_steps_batch_{batch_size}_exp{compare_index}_num{num}_k={k}.png")
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()
...
def plot_curriculum_mse_by_batch_size(mse_dict, num_exp, m_values):
    for batch_size in set(bs for bs, cs in mse_dict.keys()):
        plt.figure(figsize=(12, 6))
        current_step_values_sorted = sorted([cs for (bs, cs) in mse_dict.keys() if bs == batch_size])
        for idx, cs in enumerate(current_step_values_sorted):
            train_mse = mse_dict[(batch_size, cs)]
            x_range = range(idx * 50 + 1, (idx + 1) * 50 + 1)
            plt.plot(x_range, train_mse, label=f'm={cs}')
            if idx < len(current_step_values_sorted) - 1:
...
def plot_train_mse_recursive_steps(mse_dict, num, batch_size, m_values):
    plt.figure(figsize=(10, 6))
    ...
    plt.title(f'Train MSE Comparison for Different m, batch_size={batch_size}, {num} Rogue Wave Experiments')
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
    save_path = os.path.join(save_dir, f"train_mse_comparison_recursive_steps_batch_{batch_size}_num{num}.png")
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()
            ...
            for i in range(num_samples):
                ...
                if m >= 4:
                    ...
                    while steps_predicted < 4:
                        ...
                        step_predictions = model(history_tensor, m)               
            ...
            with torch.no_grad():
                ...
                for i in range(num_samples):
                    ...
                    while steps_predicted < target_step:
                        ...
                        history_tensor = torch.tensor(history[np.newaxis, :, :], dtype=torch.float32).to(device)
                        predictions = model(history_tensor, m)               
                        predictions = predictions.detach().cpu().numpy()               
...
def train_main():
    ...
    for csv_path, num, batch_sizes in zip(csv_paths, num_list, batch_sizes_list):
        ...
        for m_idx, m in enumerate(m_values):
            ...
            for batch_size in batch_sizes:
                ...
                if m_idx > 0:
                    ...
                    prev_model_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_v3/model_batch_{batch_size}_exp{num}_k{k}_m{prev_m}.pth"
                model = CNN(k=k, m=m, m_max=m_max).to(device)

                save_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_v3/model_batch_{batch_size}_exp{num}_k{k}_m{m}.pth"
            ...
            prev_model_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_v3/model_batch_{best_batch_size}_exp{num}_k{k}_m{m}.pth"
        ...
        for batch_size in batch_sizes:
            ...
            plot_train_mse_recursive_steps(mse_errors_dict, num, batch_size, m_values)
        
        plot_curriculum_mse_by_batch_size(mse_dict, num, m_values)

        model_paths = {}
        for m in m_values:
            model_paths[m] = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_v3/model_batch_{best_batch_size}_exp{num}_k{k}_m{m}.pth"
    ...
    for csv_path, num, idx, batch_sizes in zip(csv_paths, num_list, exp_indices, batch_sizes_list):
        ...
        for m in m_values:
            ...
            for batch_size in batch_sizes:
                ...
                save_dir_metrics = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/CNNmodels_multifinal_results_v3"
                ...
                metrics_csv = os.path.join(save_dir_metrics, f'metrics_summary_CNNv3_batch{batch_size}_m{m}_exp{num}.csv')
        ...
        for batch_size in batch_sizes:
            ...
            plot_comparison_recursive_steps(t_compare, t_start, dt_frame, predictions_dict, true_values_dict, x, batch_size, num, idx + 1, k, m_values)

        for batch_size in batch_sizes:
            mse_errors_dict = {m: mse_errors_all_m[m][batch_size] for m in m_values}
            plot_mse_recursive_steps(mse_errors_dict, times_all_m[m_values[0]], num, idx + 1, batch_size, k, m_values)
