...
def pre_data(csv_path, num, k, m):
    ...
    for i in range(num):
        ...
        y_exp = []
        for l in range(k, len(u_exp) - m + 1):
            deltas = []
            for n in range(m):
                delta = u_exp[l+n] - u_exp[l+n-1]
                deltas.append(delta)
            y_exp.append(deltas)
...
def plot_comparison_value_at_time(t_compare, t_start, dt_frame, predictions, X, x, u_min, u_max, batch_size, num, idx, k, m):
    ...
    plt.legend()
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_results_v2"
...
def compare_stage_costs_at_step4(model_paths, test_data, k, m_values, device='cuda'):
    ...
    for m in m_values:
        ...
        model = UNetModel(k=k, m=m, m_max=max(m_values)).to(device)
...
def train_main():
    ...
    for csv_path, num, batch_sizes in zip(csv_paths, num_list, batch_sizes_list):
        ...
        for m_idx, m in enumerate(m_values):
            ...
            for batch_size in batch_sizes:
                ...
                if m_idx > 0:
                    ...
                    prev_model_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_v2/model_batch_{batch_size}_exp{num}_k{k}_m{prev_m}.pth"

                model = UNetModel(k=k, m=m, m_max=max(m_values), kernel_size=kernel_size).to(device)
                save_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_v2/model_batch_{batch_size}_exp{num}_k{k}_m{m}.pth"
            ...
            prev_model_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_v2/model_batch_{best_batch_size}_exp{num}_k{k}_m{m}.pth"
        ...
        model_paths = {}
        for m in m_values:
            model_paths[m] = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_v2/model_batch_{best_batch_size}_exp{num}_k{k}_m{m}.pth"
    ...
    for csv_path, num, idx, batch_sizes in zip(csv_paths, num_list, exp_indices, batch_sizes_list):
        ...
        for m in m_values:
            for batch_size in batch_sizes:
                ...
                save_dir_metrics = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_multifinal_results_v2"
                ...
                metrics_csv = os.path.join(save_dir_metrics, f'metrics_summary_UNet_batch{batch_size}_m{m}_exp{num}.csv')
