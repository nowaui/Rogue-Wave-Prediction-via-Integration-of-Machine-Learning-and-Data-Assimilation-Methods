...
class UNetModel(nn.Module):
    def __init__(self, kernel_size=3):
        ...
        self.enc1_conv1 = nn.Conv1d(1, 16, kernel_size, padding='valid')
        ...
        self.final_conv = nn.Conv1d(16, 1, kernel_size, padding='valid')
        self.relu = nn.ReLU()
...
def train_model(model, train_loader, val_loader, device, num_epochs, batch_size, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, batch_size={batch_size}, Train Loss: {train_loss:.5f}, Val Loss: {val_loss:.5f}")
...
def plot_comparison_value_at_time(t_compare, t_start, dt_frame, predictions, X, x, u_min, u_max, batch_size, num, idx):
    ...
    save_dir = "C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_final_results"
...
def plot_individual_dataset_prediction(predictions, true_values, x, batch_size, num, idx, t_compare):
    ...
    plt.xlabel('x')
    plt.ylabel('u')
    plt.title(f'UNet prediction results, batch size = {batch_size}')
    plt.legend()
    plt.grid(True)
...
def train_main():
    ...
    for csv_path, num in zip(csv_paths, num_list):
        print(f"\ndataset: {csv_path} ({num} rogue wave experiments)")
        ...
        for batch_size in batch_sizes:
            ...
            model = UNetModel(kernel_size=kernel_size).to(device)
            save_path = f"C:/Users/DELL/Desktop/Thesis code/KdV code/Numerical sol/Final code/Final_Final_code/Unetmodels_final/model_batch_{batch_size}_exp{num}.pth"

            train_losses, val_losses = train_model(model, train_loader, val_loader, device, num_epochs=50, 
