...
def run_enkf_enhanced_unet_experiment(target_time, analysis_time, lead_time, N_ensemble, Q_d, 
    ...
    exp_name = (
        f"Results_N{N_ensemble}_Qd{Q_d}_analysis{analysis_time:.1f}s_"
        f"lead{lead_time:.1f}s_alpha{alpha}_Npred{n_prediction_ensemble}"
    )
    ...
    if not load_analysis:
        ...
        last_x_mean = x_mean_analysis[0].copy()
        last_std = std_analysis[0].copy()
        last_mse = float(mse_analysis[0])
        last_residual = float(residual_analysis[0])
        last_spread = float(spread_analysis[0])
        ...
        for k in range(0, N_t_frame_analysis, m):
            ...
            for step in range(m):
                ...
                truth_idx_for_metrics = min(end_numerical_idx, len(u_truth) - 1)
                ...
                if z_k is not None:
                    ...
                    x_mean, std = enkf_unet.get_mean_and_spread(xi_ensemble)
                    mse_value = np.mean((x_mean - u_truth[truth_idx_for_metrics])**2)
                    residual_value = np.mean(u_truth[truth_idx_for_metrics] - x_mean)
                    spread_value = np.mean(std)
                    
                    last_x_mean = x_mean.copy()
                    last_std = std.copy()
                    last_mse = float(mse_value)
                    last_residual = float(residual_value)
                    last_spread = float(spread_value)
                else:
                    x_mean = last_x_mean
                    std = last_std
                    mse_value = last_mse
                    residual_value = last_residual
                    spread_value = last_spread
                
                                                                                                           
                x_mean_to_write = x_mean if z_k is not None else last_x_mean
                std_to_write = std if z_k is not None else last_std
                
                for num_idx in range(start_numerical_idx, min(end_numerical_idx + 1, len(x_mean_analysis))):
                    if num_idx < len(x_mean_analysis):
                        x_mean_analysis[num_idx] = x_mean_to_write
                        std_analysis[num_idx] = std_to_write
                        mse_analysis[num_idx] = mse_value
                        residual_analysis[num_idx] = residual_value
                        spread_analysis[num_idx] = spread_value
    ...
    localization_radius_pred = L / 8.0
...
def create_enhanced_unet_figures(target_time, analysis_time, lead_time, N_ensemble, Q_d, 
    ...
    plt.title(
        f'EnKF-UNet Prediction vs Truth at t = {selected_time_s:.2f} ({assimilation_forecast_note})',
        fontsize=16,
        fontweight='bold'
    )
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, f'prediction_vs_truth_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
    ...
    plt.axvline(x=selected_time_s, color='red', linestyle='--', linewidth=2, label=f'Target time (t={selected_time_s:.2f})')
    plt.xlabel('Time t', fontsize=14)
    plt.xlim(0.0, selected_time_s)
                          
    plt.ylabel('Mean Squared Error (MSE)', fontsize=14)
    plt.title(
        f'MSE Evolution ({assimilation_forecast_note})',
        fontsize=16,
        fontweight='bold'
    )
    plt.legend(fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, f'mse_evolution_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
    ...
    plt.ylabel('Wave Amplitude u', fontsize=14)
    plt.title(
        f'EnKF-UNet with Error Bands at t = {selected_time_s:.2f} '
        f'(Analysis until t={analysis_time:.1f}; {assimilation_forecast_note})',
        fontsize=16,
        fontweight='bold'
    )
    plt.legend(fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, f'error_bands_t{selected_time_s:.1f}_{title_suffix}.png'), dpi=300)
    ...
    phase_info = current_phase_label
    ...
    plt.ylabel('Wave Amplitude u', fontsize=14)
    plt.title(
        f'All Ensemble Members vs Truth at t = {selected_time_s:.2f} ({phase_info})',
        fontsize=16,
        fontweight='bold'
    )
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, f'ensemble_members_vs_truth_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
    ...
    phase_info_env = current_phase_label
    ...
    plt.ylabel('Wave Amplitude u', fontsize=14)
    plt.title(
        f'EnKF-UNet with Min–Max Error Bands at t = {selected_time_s:.2f} ({phase_info_env}) '
        f'(Analysis until t={analysis_time:.1f}; {assimilation_forecast_note})',
        fontsize=16,
        fontweight='bold'
    )
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, f'prediction_with_minmax_envelope_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
    ...
    for i, alpha in enumerate(alpha_values):
        for j, n_pred in enumerate(n_prediction_values):
            ...
            try:
                ...
                plt.xlabel('Spatial Position x', fontsize=14)
                plt.ylabel('Wave Amplitude u', fontsize=14)
                plt.title(f'EnKF-UNet Prediction vs Truth at t = {selected_time_s:.2f} ({assimilation_forecast_note})', fontsize=16, fontweight='bold')
                plt.legend(fontsize=12)
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(exp_dir, f'prediction_vs_truth_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
                ...
                if t_plot.size == 0 or abs(t_plot[-1] - selected_time_s) > 1e-12:
                    t_plot = np.append(t_plot, selected_time_s)
                    mse_plot = np.append(mse_plot, results['mse_total'][max(0, min(len(results['mse_total']) - 1, int(round(selected_time_s / dt_numerical))))])
                plt.figure(figsize=(12, 6))
                plt.semilogy(t_plot, mse_plot, 'g-', lw=2.5, label='MSE')
                plt.axvline(x=analysis_time, color='blue', ls='--', lw=2, alpha=0.7, label=f'Analysis t={analysis_time:.1f}')
                plt.axvline(x=selected_time_s, color='red', ls='--', lw=2, label=f'Target t={selected_time_s:.2f}')
                plt.xlabel('Time t', fontsize=14)
                plt.ylabel('Mean Squared Error (MSE)', fontsize=14)
                plt.title(f'MSE Evolution ({assimilation_forecast_note})', fontsize=16, fontweight='bold')
                plt.legend(fontsize=12)
                plt.grid(True, alpha=0.3)
                mse_series = results['mse_total']
                if mse_series is not None and len(mse_series) > 0:
                    finite_mask = np.isfinite(mse_series)
                    if np.any(finite_mask):
                        rmse_final = float(np.sqrt(mse_series[finite_mask][-1]))
                        ax = plt.gca()
                        ax.text(0.98, 0.92, f'Final RMSE = {rmse_final:.6f}', transform=ax.transAxes,
                                ha='right', va='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.7))
                plt.tight_layout()
                ...
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(exp_dir, f'error_bands_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
                plt.close()

                                                          
                if selected_idx <= analysis_idx:
                    ensemble_snapshot = ensemble_da
                    phase_info = assimilation_phase_label
                else:
                    ensemble_snapshot = ensemble_pred
                    phase_info = forecast_phase_label
                ...
                plt.xlabel('Spatial Position x', fontsize=14)
                plt.ylabel('Wave Amplitude u', fontsize=14)
                plt.title(f'All Ensemble Members vs Truth at t = {selected_time_s:.2f} ({phase_info})', fontsize=16, fontweight='bold')
                ...
                plt.ylabel('Wave Amplitude u', fontsize=14)
                plt.title(
                    f'EnKF-UNet with Min–Max Error Bands at t = {selected_time_s:.2f} ({phase_info}) '
                    f'(Analysis until t={analysis_time:.1f}; {assimilation_forecast_note})',
                    fontsize=16,
                    fontweight='bold'
                )
                plt.legend(fontsize=12, loc='upper right'); plt.grid(True, alpha=0.3); plt.tight_layout()
                plt.savefig(os.path.join(exp_dir, f'prediction_with_minmax_envelope_t{selected_time_s:.2f}_{title_suffix}.png'), dpi=300)
                plt.close()
...
def save_localization_heatmaps(results, save_dir, alpha=None, n_pred=None):
    ...
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'localization_matrix{tag}.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'ensemble_covariance_matrix{tag}.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'ensemble_covariance_after_localization{tag}.png'), dpi=300, bbox_inches='tight')
    plt.close()
