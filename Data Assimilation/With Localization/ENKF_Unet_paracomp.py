...
def load_truth_solution(store_path, required_rows, M, dtype=np.float32, chunk_rows=5000):
    """
    Load the required portion of the truth solution from CSV without exhausting RAM.
    Creates a cached .npy file (memmap) the first time it is needed.
    """
    if not os.path.exists(store_path):
        raise ValueError(f"Truth data file not found: {store_path}")

    base, ext = os.path.splitext(store_path)
    cache_path = f"{base}_rows{required_rows}_M{M}.npy"

    if os.path.exists(cache_path):
        data = np.load(cache_path, mmap_mode='r')
        if data.shape != (required_rows, M):
                                                                                   
            os.remove(cache_path)
        else:
            print(f"Loading cached truth data from {cache_path}")
            return data

    print(f"Caching truth data to {cache_path} to avoid large in-memory CSV loads...")
    mmap_arr = open_memmap(cache_path, mode='w+', dtype=dtype, shape=(required_rows, M))
    rows_written = 0
    ...
    for chunk in pd.read_csv(
        ...
        if rows_written >= required_rows:
            break
        chunk_values = chunk.to_numpy(dtype=dtype, copy=False)
        if chunk_values.shape[1] != M:
            del mmap_arr
            os.remove(cache_path)
            raise ValueError(
                f"Truth data column mismatch. Expected {M}, got {chunk_values.shape[1]}"
            )
        rows_to_copy = min(chunk_values.shape[0], required_rows - rows_written)
        mmap_arr[rows_written:rows_written + rows_to_copy] = chunk_values[:rows_to_copy]
        rows_written += rows_to_copy
    ...
    if rows_written < required_rows:
        os.remove(cache_path)
        raise ValueError(
            f"Truth CSV contains only {rows_written} rows, "
            f"but {required_rows} were requested."
        )
...
class mKdVSystem:    
    ...
    def F(self, xi_ensemble):
        ...
        F_ensemble = np.zeros((M, N), dtype=np.float32)
        
                                      
        xi_ext = np.zeros((M + 4, N), dtype=np.float32)
...
class EnKF_UNet:   
    ...
    def create_gaussian_Q(self, Q_d):
        Q = np.zeros((self.M_dim, self.M_dim), dtype=np.float32)
    ...
    def initialize_enkf(self, x0, P0_std=0.1):
        xi_ensemble = np.zeros((self.M_dim, self.N), dtype=np.float32)
...
def run_enkf_enhanced_unet_experiment(target_time, analysis_time, lead_time, N_ensemble, Q_d, 
                                      ...
                                      obs_ratio=0.1, localization_radius=None):
    ...
    base_dir = r"C:\Users\DELL\Desktop\Thesis code\KdV code\Numerical sol\Final code\Final_Final_code\Data_assimilation\Covariance Localization\Unet comp"
    os.makedirs(base_dir, exist_ok=True)
    
                                                                             
    exp_name = (
        f"N{N_ensemble}_Q{Q_d}_a{analysis_time:.1f}_l{lead_time:.1f}_np{n_prediction_ensemble}_a{alpha}_obs{obs_ratio:.2f}"
    )
    ...
    enable_load_analysis_from_disk = False
    enable_save_analysis_to_disk = False
    
                                                     
    load_analysis = False
    if enable_load_analysis_from_disk and save_data and all(os.path.exists(f) for f in analysis_files.values()):
        try:
            ...
            if (stored_params['target_time'] == target_time and
                ...
                stored_params['Q_d'] == Q_d and
                (('obs_ratio' in stored_params and stored_params['obs_ratio'] == obs_ratio) or ('obs_ratio' not in stored_params and abs(obs_ratio-0.1)<1e-12))):
                load_analysis = True
    ...
    if load_analysis:
        ...
        try:
            ...
            if os.path.exists(store_path):
                ...
                u_truth = load_truth_solution(store_path, required_rows, M)
            else:
    ...
    if not load_analysis:
        ...
        H = np.zeros((n_obs, M), dtype=np.float32)
        ...
        if print_covariance_info:
            try:
                enkf_unet.debug_save_dir = analysis_dir
            except Exception:
                pass
        ...
        if save_data and enable_save_analysis_to_disk:
    ...
    localization_radius_pred = localization_radius if localization_radius is not None else (L / 8.0)
    enkf_pred = EnKF_UNet(unet_system, n_prediction_ensemble, Q_std=Q_std_pred, R_std=obs_noise_std, Q_d=Q_d,
                          localization_radius=localization_radius_pred, dx=dx)
    print(f"[EnKF-UNet] Prediction phase using localization radius: {localization_radius_pred}")
    if print_covariance_info:
        try:
    ...
    return {
        ...
        'alpha': alpha,
        'obs_ratio': obs_ratio,
        'localization_radius': localization_radius_pred,
...
def run_enkf_unet_experiment(target_time, N_ensemble, Q_d):
    ...
    if os.path.exists(store_path):
        ...
        required_rows = N_t_numerical + 1
        try:
            u_truth_full = load_truth_solution(store_path, required_rows, M)
            u_truth = u_truth_full[:required_rows]
            u0_truth = np.array(u_truth[0], copy=True)
        except ValueError as exc:
            print(f"Failed to load cached truth solution: {exc}")
            u_truth = None
    else:
        u_truth = None
    ...
    if u_truth is None:
        ...
        u_truth = np.zeros((N_t_numerical + 1, M), dtype=np.float32)
        u_truth[0] = u0_truth
...
def create_enhanced_unet_figures(target_time, analysis_time, lead_time, N_ensemble, Q_d, 
                                selected_members=None, alpha=1.0, n_prediction_ensemble=None, 
                                save_data=True, time_chosen=None, obs_ratio=0.1):
    results = run_enkf_enhanced_unet_experiment(target_time, analysis_time, lead_time, N_ensemble, Q_d,
                                               selected_members, alpha, n_prediction_ensemble, save_data, obs_ratio=obs_ratio)
    ...
    title_suffix = f"N{N_ensemble_actual}_alpha{alpha_actual}_npred{n_pred_actual}_obs{results.get('obs_ratio', obs_ratio):.2f}"
...
def compare_obs_ratio_unet(target_time, analysis_time, lead_time, N_ensemble, Q_d, 
                           obs_ratio_values, selected_members=None, 
                           save_data=True, time_chosen=None, n_prediction_ensemble=10,
                           obs_ratio_to_radii=None):
    base_dir = r"C:\\Users\\DELL\\Desktop\\Thesis code\\KdV code\\Numerical sol\\Final code\\Final_Final_code\\Data_assimilation\\Covariance Localization\\Unet comp"
    os.makedirs(base_dir, exist_ok=True)
    ...
    compare_name = f"cmpObs_N{N_ensemble}_Q{Q_d}_a{analysis_time:.1f}_l{lead_time:.1f}_np{n_prediction_ensemble}"
    ...
    multi_radius = isinstance(obs_ratio_to_radii, dict) and len(obs_ratio_to_radii) > 0
    max_cols = max((len(v) for v in (obs_ratio_to_radii.values() if multi_radius else [[]])), default=1)
    rmse_grid = np.full((len(obs_ratio_values), max_cols), np.nan)
    print(f"Running {len(obs_ratio_values)} obs_ratio values...")
    
    combination_idx = 0
    for i, obs_ratio in enumerate(obs_ratio_values):
        radii_list = obs_ratio_to_radii.get(obs_ratio, [None]) if multi_radius else [None]
        for j, r_loc in enumerate(radii_list):
            combination_idx += 1
            print(f"\nRunning {combination_idx}/{len(obs_ratio_values)*(len(radii_list))}: obs_ratio={obs_ratio}, R={r_loc if r_loc is not None else 'default'}, α=1.0, n_pred={n_prediction_ensemble}")
            ...
            results = run_enkf_enhanced_unet_experiment(
                ...
                selected_members, 1.0, n_prediction_ensemble, save_data=True, print_covariance_info=print_cov_info,
                obs_ratio=obs_ratio, localization_radius=r_loc
            )
            ...
            save_localization_heatmaps(results, exp_dir, alpha=1.0, n_pred=n_prediction_ensemble)

                                                                      
            try:
                time_vec = np.arange(len(results['mse_total'])) * results['dt_numerical']
                mse_series = results['mse_total']
                ...
                os.makedirs(exp_dir, exist_ok=True)
                loc_suffix = f"_r{r_loc:.2f}" if isinstance(r_loc, (int, float)) else ""
                ts_name = f'time_series_obs{obs_ratio:.2f}{loc_suffix}_npred{n_prediction_ensemble}.csv'
            ...
            try:
                ...
                alpha_actual = results['alpha']
                n_pred_actual = results['n_prediction_ensemble']
                obs_tag = results.get('obs_ratio', obs_ratio)
                loc_tag = results.get('localization_radius', r_loc)
                loc_str = f"_r{loc_tag:.2f}" if isinstance(loc_tag, (int, float)) else ""
                title_suffix = f"N{N_ensemble_actual}_alpha{alpha_actual}_npred{n_pred_actual}_obs{obs_tag:.2f}{loc_str}"
    ...
    try:
        ...
        res0 = results_dict[first_key]['results']
        x0 = res0.get('x')
        dx0 = float(x0[1] - x0[0]) if x0 is not None and len(x0) > 1 else 0.1
        Ldomain = float(x0[-1] - x0[0] + dx0) if x0 is not None else 50.0
    except Exception:
        Ldomain = 50.0
    ...
    col_labels = []
    if multi_radius and isinstance(obs_ratio_to_radii, dict):
        ref_obs = obs_ratio_values[0] if len(obs_ratio_values) > 0 else None
        ref_list = obs_ratio_to_radii.get(ref_obs, []) if ref_obs is not None else []
        for j in range(rmse_grid.shape[1]):
            if j < len(ref_list) and ref_list[j] not in (None, 0):
                col_labels.append(_fmt_r_label(ref_list[j]))
            elif not multi_radius:
                col_labels.append(_fmt_r_label(Ldomain / 8.0))
            else:
                col_labels.append("")
    else:
                            
        col_labels = [_fmt_r_label(Ldomain / 8.0)]
    plt.xticks(range(rmse_grid.shape[1]), col_labels)
    plt.xlabel('Localization radius (L/xx)', fontsize=14)
                                     
    plt.yticks(range(len(obs_ratio_values)), [f"{v:.2f}" for v in obs_ratio_values])
    plt.ylabel('obs_ratio', fontsize=14)
    plt.title(f'RMSE vs obs_ratio at t = {selected_time_s:.2f}s (α=1, N_pred={n_prediction_ensemble})', fontsize=14, fontweight='bold')
    
                          
    for i, obs_ratio in enumerate(obs_ratio_values):
        cols = range(rmse_grid.shape[1]) if multi_radius else [0]
        for j in cols:
            txt = f"{rmse_grid[i, j]:.4f}" if np.isfinite(rmse_grid[i, j]) else 'nan'
                                                                      
            label = txt
            plt.text(j, i, label, ha='center', va='center', color='white', fontweight='bold')
    
    plt.tight_layout()
    plt.savefig(os.path.join(compare_dir, f'rmse_heatmap_obs_ratio_t_{selected_time_s:.2f}s.png'), 
                dpi=300, bbox_inches='tight')
                            
    
                                                                  
    metrics_rows = []
    for (obs_ratio, r_loc), res in results_dict.items():
        dt_num = res['dt_numerical']
        sel_idx = int(round(selected_time_s / dt_num))
        sel_idx = max(0, min(sel_idx, len(res['u_truth']) - 1))
        truth_sel = res['u_truth'][sel_idx]
        est_sel = res['x_mean_total'][sel_idx]
        ...
        metrics_rows.append({
            'obs_ratio': obs_ratio,
            'localization_radius': res.get('localization_radius', r_loc),
            'N_ensemble': res['N_ensemble'],
            'alpha': res['alpha'],
            'n_prediction_ensemble': res['n_prediction_ensemble'],
            'Q_d': Q_d,
            'rmse_domain': rmse,
            'peak_amplitude_error': peak_amp_error,
            'mean_abs_error_peak_region': mean_abs_err_peak_region,
            'final_mse': res['mse_total'][-1] if len(res['mse_total'])>0 else np.nan,
            'final_spread': res['spread_total'][-1] if len(res['spread_total'])>0 else np.nan
        })
    metrics_df = pd.DataFrame(metrics_rows)
    metrics_csv = os.path.join(compare_dir, 'metrics_obs_ratio.csv')
    metrics_df.to_csv(metrics_csv, index=False)

                                                                                  
    if not metrics_df.empty:
        summary_cols = ['rmse_domain', 'peak_amplitude_error', 'mean_abs_error_peak_region']
        summary = metrics_df.groupby('obs_ratio', as_index=False)[summary_cols].mean().sort_values('obs_ratio')

        print("\n=== Metrics vs obs_ratio (fixed localization radius) ===")
        for row in summary.itertuples(index=False):
            print(f"obs_ratio={row.obs_ratio:.2f} -> RMSE={row.rmse_domain:.6f}, "
                  f"Peak error={row.peak_amplitude_error:.6f}, "
                  f"Mean abs. (peak)={row.mean_abs_error_peak_region:.6f}")

        plt.figure(figsize=(10, 6))
        plt.plot(summary['obs_ratio'], summary['rmse_domain'], 'o-', linewidth=2, markersize=6, color='tab:blue')
        plt.xlabel('Observation ratio', fontsize=12)
        plt.ylabel('RMSE (domain)', fontsize=12)
        radius_values = metrics_df['localization_radius'].dropna().unique()
        radius_note = ""
        if len(radius_values) == 1:
            radius_note = f" (R={radius_values[0]:.2f})"
        plt.title(f'RMSE vs obs_ratio at analysis t={analysis_time:.2f}{radius_note}', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        rmse_plot_path = os.path.join(compare_dir, 'rmse_vs_obs_ratio.png')
        plt.savefig(rmse_plot_path, dpi=300, bbox_inches='tight')
        plt.close()

        print(f"\nSaved obs_ratio metrics CSV to {metrics_csv}")
        print(f"Saved RMSE vs obs_ratio plot to {rmse_plot_path}")
    
    print(f"obs_ratio comparison results saved to {compare_dir}")
...
def save_localization_heatmaps(results, save_dir, alpha=None, n_pred=None):
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        print("Matplotlib not available for plotting")
    ...
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'localization_matrix{tag}.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    P_f = P_f.astype(np.float32, copy=False)
    plt.figure(figsize=(6, 5))
    im = plt.imshow(P_f, cmap='viridis', aspect='auto', vmin=vmin, vmax=vmax, interpolation='nearest')
    plt.colorbar(im, label='Covariance')
    plt.title('Ensemble covariance matrix (P_f)')
    plt.xlabel('j')
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'ensemble_covariance_matrix{tag}.png'), dpi=150, bbox_inches='tight')
    plt.close()

                                                       
    P_f_loc = P_f_loc.astype(np.float32, copy=False)
    plt.figure(figsize=(6, 5))
    im = plt.imshow(P_f_loc, cmap='viridis', aspect='auto', vmin=vmin, vmax=vmax, interpolation='nearest')
    plt.colorbar(im, label='Covariance (localized)')
    plt.title('Ensemble covariance after localization (P_f ∘ rho)')
    plt.xlabel('j')
    plt.ylabel('i')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'ensemble_covariance_after_localization{tag}.png'), dpi=150, bbox_inches='tight')
    plt.close()

if __name__ == "__main__":        
  
    obs_ratio_values = [0.01, 0.02, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40]

    L = 50.0
    fixed_radius = L / 10.0
    obs_ratio_to_radii = {ratio: [fixed_radius] for ratio in obs_ratio_values}
    results_dict_combo, compare_dir_combo = compare_obs_ratio_unet(
        target_time=15.0,
        analysis_time=6.0,
        lead_time=7.8,
        N_ensemble=150,
        Q_d=10,
        obs_ratio_values=obs_ratio_values,
        selected_members=None,
        save_data=True,
        time_chosen=7.8,
        n_prediction_ensemble=150,
        obs_ratio_to_radii=obs_ratio_to_radii
    )
